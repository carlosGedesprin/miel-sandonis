<?php

namespace src\controller\entity;

use DateTime;
use DateTimeZone;

class userController
{
    use repository\userRepositoryController;
    use verifications\userVerificationController;

    protected $env;
    protected $logger;
    protected $logger_err;
    protected $startup;
    protected $db;
    protected $utils;
    protected $session;
    protected $lang;
    
    protected $user_connected;
    protected $group_connected;

    private $myfile;

    private $crypt_options = array(
        'cost' => 12,
    );

    private $table = "user";    
    private $reg = array( 
                            'id'                => '',
                            'account'           => NULL,
                            'role'              => NULL,
                            'user_key'          => NULL,
                            'password'          => NULL,
                            'email'             => NULL,
                            'name'              => NULL,
                            'lastlogin'         => NULL,
                            'attempt'           => NULL,
                            'locale'            => NULL,
                            'activation_key'    => NULL,
                            'change_password_key' => NULL,
                            'show_to_staff'     => '1',
                            'active'            => '0',
                        );

    // NO TOCAR: EXISTE UNA RAZON PORQUE NO CARGAMOS baseController
    public function __construct()
    {
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/entity_user.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $arguments = func_get_args();
        if(!empty($arguments))
            foreach($arguments[0] as $key => $property)
                if(property_exists($this, $key))
                    $this->{$key} = $property;
//$txt = 'argumets OK ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        $this->user_connected = ( isset($_SESSION[$this->session->config['cookies_prefix'].'_CurUser']) )? $_SESSION[$this->session->config['cookies_prefix'].'_CurUser'] : '';
        $this->group_connected = ( isset( $_SESSION[$this->session->config['cookies_prefix'].'_UserGroup']) )? $_SESSION[$this->session->config['cookies_prefix'].'_UserGroup'] : '';
//$txt = 'this OK ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = __METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
    }

    /**
     *
     * Get table name
     *
     */
    public function getTableName()
    {
        return $this->table;
    }

    /**
     *
     * Get user from his id
     *
     */
    public function getRegbyId( $id )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Id ==========> ('.$id.')'.PHP_EOL; fwrite($this->myfile, $txt);
        if ( !$id ) return false;

        $filter = array( 'id' => $id  );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getRegFromDB( $filter );
    }

    /**
     *
     * Get user by his email
     *
     */
    public function getRegByUserKey( $user_key )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'User key ==========> ('.$user_key.')'.PHP_EOL; fwrite($this->myfile, $txt);
        if ( !$user_key ) return false;

        $filter = array( 'user_key' => $user_key );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getRegFromDB( $filter );
    }

    /**
     *
     * Get user by his email
     *
     */
    public function getRegByEmail( $email )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Email ==========> ('.$email.')'.PHP_EOL; fwrite($this->myfile, $txt);
        if ( !$email ) return false;

        $filter = array( 'email' => $email  );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getRegFromDB( $filter );
    }

    /**
     *
     * Get user by his activation key
     *
     */
    public function getRegByActivationKey( $activationkey )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'ActivationKey ==========> ('.$activationkey.')'.PHP_EOL; fwrite($this->myfile, $txt);
        if ( !$activationkey ) return false;

        $filter = array( 'activation_key' => $activationkey  );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getRegFromDB( $filter );
    }

    /**
     *
     * Get reg
     *
     */
    private function getRegFromDB( $filter )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Filter ==========> '.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($filter, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);

        if ( $item = $this->db->fetchOne( $this->table, '*', $filter ) )
        {
            $this->reg = array_merge( $this->reg, $item );

            $this->loadSpecialFields();

//$txt = 'reg found==========> ('.$this->getId().')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->getReg(), TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return true;
        }
        else
        {
//$txt = 'reg NOT found ==========> '.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return false;
        }
    }

    /**
     *
     * Persist to db
     *
     */
    public function persistORL()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==> ('.$this->table.') User ==> ('.$this->user_connected.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);

        $this->setSpecialFields();

        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->updateArrayORL( $this->table, $this->user_connected, 'id', $this->reg['id'], $this->reg );
//$txt = 'reg updated ==> ORL'.PHP_EOL; fwrite($this->myfile, $txt);
        }
        else
        {
            unset( $this->reg['id'] );
            $this->setId( $this->db->insertArrayORL( $this->table, $this->user_connected, $this->reg ) );
        }

        $this->loadSpecialFields();

//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getId();
    }

    /**
     *
     * Persist to db
     *
     */
    public function persist()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==> ('.$this->table.') User ==> ('.$this->user.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);

        $this->setSpecialFields();

        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->updateArray( $this->table, 'id', $this->reg['id'], $this->reg );
//$txt = 'reg updated ==> ORL'.PHP_EOL; fwrite($this->myfile, $txt);
        }
        else
        {
            unset( $this->reg['id'] );
            $this->setId( $this->db->insertArray( $this->table, $this->reg ) );
        }

        $this->loadSpecialFields();

//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getId();
    }

    /**
     *
     * Delete this record
     *
     */
    public function deleteORL()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==========> ('.$this->table.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->deleteORL( $this->table, $this->user_connected, 'id', $this->getId() );
//$txt = 'reg deleted ==========> '.$this->reg['id'].PHP_EOL; fwrite($this->myfile, $txt);
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return true;
        }
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return false;
    }

    /**
     *
     * Delete this record
     *
     */
    public function delete()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==========> ('.$this->table.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->delete( $this->table, 'id', $this->getId() );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return true;
        }
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return false;
    }

    public function setReg( $reg ) { $this->reg = array_merge( $this->reg, $reg );  }
    public function setId( $id ) { $this->reg['id'] = $id;  }
    public function setAccount( $account ) { $this->reg['account'] = $account;  }
    public function setRole( $role ) { $this->reg['role'] = $role;  }
    public function setUserKey( $userkey ) { $this->reg['user_key'] = $userkey;  }
    public function setPassword( $password ) { $this->reg['password'] = $password;  }
    public function setEmail( $email ) { $this->reg['email'] = $email;  }
    public function setName( $name ) { $this->reg['name'] = $name;  }
    public function setLastLogin( $lastlogin ){ $this->reg['lastlogin'] = $lastlogin; }
    public function setAttempt( $attempt ) { $this->reg['attempt'] = $attempt; }
    public function setLocale( $locale ) { $this->reg['locale'] = $locale; }
    public function setActivationKey( $activationkey ) { $this->reg['activation_key'] = $activationkey; }
    public function setChangePasswordKey($changepasswordkey) { $this->reg['change_password_key'] = $changepasswordkey;}
    public function setShowToStaff( $show_to_staff ) { $this->reg['show_to_staff'] = $show_to_staff; }
    public function setActive( $active ) { $this->reg['active'] = $active; }

    public function getReg() { return $this->reg; }
    public function getAll( $filter, $extra ) { return $this->db->fetchAll($this->table, '*', $filter, $extra); }
    public function getId() { return $this->reg['id']; }
    public function getAccount() { return $this->reg['account']; }
    public function getRole() { return $this->reg['role']; }
    public function getUserKey() { return $this->reg['user_key']; }
    public function getPassword() { return $this->reg['password']; }
    public function getEmail() { return $this->reg['email']; }
    public function getName() { return $this->reg['name']; }
    public function getLastlogin() { return $this->reg['lastlogin']; }
    public function getAttempt() {return $this->reg['attempt']; }
    public function getLocale() { return $this->reg['locale']; }
    public function getActivationKey() { return $this->reg['activation_key']; }
    public function getChangePasswordKey() { return $this->reg['change_password_key']; }
    public function getShowToStaff() { return $this->reg['show_to_staff']; }
    public function getActive() { return $this->reg['active']; }
    
    /**
     *
     * Change special fields after load from database
     */
    private function loadSpecialFields()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        $this->reg['lastlogin'] = ( $this->reg['lastlogin'] == '' )? NULL : DateTime::createFromFormat('Y-m-d H:i:s', $this->reg['lastlogin'], new DateTimeZone($this->session->config['time_zone']));
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
    }

    /**
     *
     * Change special fields before save to database
     */
    private function setSpecialFields()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        $this->reg['role'] = ( empty($this->reg['role']) )? NULL : $this->reg['role'];
        $this->reg['user_key'] = ( empty($this->reg['user_key']) )? NULL : $this->reg['user_key'];
        $this->reg['email'] = ( empty($this->reg['email']) )? NULL : $this->reg['email'];

        $this->reg['lastlogin'] = ( $this->reg['lastlogin'] == '' )? NULL : $this->reg['lastlogin']->format('Y-m-d H:i:s');
        $this->reg['show_to_staff'] = ( $this->reg['show_to_staff'] == '' )? '0' : $this->reg['show_to_staff'];
        $this->reg['active'] = ( empty($this->reg['active']) )? '0' : $this->reg['active'];
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
    }
}
