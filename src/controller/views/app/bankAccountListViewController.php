...
class bankAccountListViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/bank_accounts', name='app_bank_accounts')
     */
    public function itemslistAction()
    {
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/bankAccountListViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
...
//fwrite($this->myfile, print_r($this->pagination, TRUE));
        $this->list_filters['name']['caption'] = $this->lang['BANK_ACCOUNT_NAME'];
        $this->list_filters['name']['placeholder'] = $this->lang['BANK_ACCOUNT_NAME'];
...
//fclose($this->myfile);
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/bankAccounts.html.twig', array(
