...
class countryEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/country/edit/id', name='app_country_edit')
     *
     * @param POST
     *
...
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/countryEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
...
//fwrite($this->myfile, print_r($vars, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        ...
        $form_action = $this->folder.'/country/editor';
        ...
        $reg->setCode2a( $this->utils->request_var( 'code_2a', ($vars['code_2a'] == '0')? '' : $vars['code_2a'], 'ALL') );
        $reg->setCode3a( $this->utils->request_var( 'code_3a', '', 'ALL') );
        $reg->setCode3n( $this->utils->request_var( 'code_3n', '', 'ALL', true) );
        $reg->setFlag( $this->utils->request_var( 'flag', '', 'ALL') );
        $reg->setPhoneCode( $this->utils->request_var( 'phone_code', '', 'ALL') );
        ...
        //$reg_name->setCountryCode2a( $reg->getCode2a() );
...
//fwrite($this->myfile, print_r($data['names'], TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        ...
        $files_folder = DOCUMENT_ROOT_PATH.'/countries/';
        ...
        $files = array(
            '1' => array (
                ...
                'image_error_text' => $this->lang['ERR_COUNTRY_FLAG_NEEDED'],
                'file_entity' => 'reg',
...
//fwrite($this->myfile, print_r($data['names'], TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
            ...
            {
//$txt = 'Name to check ====== lang_code_2a ('.$value['lang_code_2a'].') name ('.$value['country_name'].')'.PHP_EOL; fwrite($this->myfile, $txt);
                $reg_name->setCountryCode2a( $reg->getCode2a() );
                $reg_name->setLang2a( $value['lang_code_2a'] );
                $reg_name->setName( $value['country_name'] );
...
//$txt = 'Deleting names in table =========='.PHP_EOL; fwrite($this->myfile, $txt);
                //$reg_name->deleteByCountryCode2a( $reg->getCode2a() );
...
//$txt = 'Lang to treat ====> ('.$row['code_2a'].')'.PHP_EOL; fwrite($this->myfile, $txt);
                    ...
                    $reg_name->setName( $data['names'][$row['code_2a']]['country_name'] );
...
//$txt = '================= Moving images from temp to files destiny folder end =============================================='.PHP_EOL.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['COUNTRY_SAVED'];
...
//$txt = 'Country can not be saved reg -> Status '.$check_reg['status'].PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($check_reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Country can not be saved reg_name -> '.$check_reg_name['status'].PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($check_reg_name, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
...
//$txt = 'Lang name from api for Lang code 2a ('.$api_data['lang_code_2a'].') code 2a ('.$api_data['lang_2a'].')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($lang_name, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
                            ...
                            $response['errors'][$error_key]['msg'] = sprintf($this->lang[$error['msg'][0]], $lang_name);
                            //$response['errors'][$error_key]['msg'] = sprintf($this->lang[$error['msg'][0]], $error['msg'][1]);
                        }
                        else
                        {
...
//$txt = 'Lang to treat ('.$lang_value['code_2a'].')'.PHP_EOL; fwrite($this->myfile, $txt);
                    $data['names'][$lang_value['code_2a']]['lang_code_2a'] = $lang_value['code_2a'];
                    $data['names'][$lang_value['code_2a']]['country_name'] = '';
                }
                //********* Names treatment openges end ******************
            }
            else
            {
//$txt = '========== EDIT =========='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Reg to edit =========> '.$reg->getCode2a().PHP_EOL; fwrite($this->myfile, $txt);

                if ( $reg->getRegbyCode2a( $reg->getCode2a() ) )
                {
