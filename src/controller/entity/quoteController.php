<?php

namespace src\controller\entity;

use src\controller\baseController;

use DateTime;
use DateTimeZone;

class quoteController extends baseController
{
    use repository\quoteRepositoryController;

    private $table = 'quote';
    private $reg = array(
                            'id'            => '',
                            'account'       => NULL,
                            'date'          => NULL,
                            'quote_key'     => NULL,
                            'net'           => '0',
                            'vat_amount'    => '0',
                            'total_to_pay'  => '0',
                            'quote_type'    => NULL,
                            'invoice'       => NULL,
                            'payment_type'  => NULL,
                            'payment_method'     => NULL,
                            'payment_origin'     => NULL,
                            'payment_reference'  => NULL,
                        );

    /**
     *
     * Get table name
     *
     */
    public function getTableName()
    {
        return $this->table;
    }

    /**
     *
     * Get quote from his id
     *
     */
    public function getRegbyId( $id )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Id ==========> ('.$id.')'.PHP_EOL; fwrite($this->myfile, $txt);
        if ( !$id ) return false;

        $filter = array( 'id' => $id  );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getRegFromDB( $filter );
    }

    /**
     *
     * Get quote from his key
     *
     */
    public function getRegbyQuoteKey( $quote_key )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Quote key ==========> ('.$quote_key.')'.PHP_EOL; fwrite($this->myfile, $txt);
        if ( !$quote_key ) return false;

        $filter = array( 'quote_key' => $quote_key  );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getRegFromDB( $filter );
    }

    /**
     *
     * Get quote from his Date and Account
     *
     */
    public function getRegbyDateAndAccount( $date, $account_id )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Id ==========> ('.$id.')'.PHP_EOL; fwrite($this->myfile, $txt);
        if ( !$date || !$account_id ) return false;

        $filter = array( 'date' => $date, 'account' => $account_id  );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getRegFromDB( $filter );
    }

    /**
     *
     * Get reg
     *
     */
    private function getRegFromDB( $filter )
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Filter ==========> ('.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($filter, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        if ( $item = $this->db->fetchOne( $this->table, '*', $filter ) )
        {
            $this->reg = array_merge( $this->reg, $item );

            $this->loadSpecialFields();

//$txt = 'reg found==========> ('.$this->getId().')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->getReg(), TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return true;
        }
        else
        {
//$txt = 'reg NOT found ==========> '.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return false;
        }
    }

    /**
     *
     * Persist to db
     *
     */
    public function persistORL()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==> ('.$this->table.') User ==> ('.$this->user.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);

        $this->setSpecialFields();

        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->updateArrayORL( $this->table, $this->user, 'id', $this->reg['id'], $this->reg );
//$txt = 'reg updated ==> ORL'.PHP_EOL; fwrite($this->myfile, $txt);
        }
        else
        {
            unset( $this->reg['id'] );
            $this->setId( $this->db->insertArrayORL( $this->table, $this->user, $this->reg ) );
        }

        $this->loadSpecialFields();

//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getId();
    }

    /**
     *
     * Persist to db
     *
     */
    public function persist()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==> ('.$this->table.') User ==> ('.$this->user.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);

        $this->setSpecialFields();

        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->updateArray( $this->table, 'id', $this->reg['id'], $this->reg );
//$txt = 'reg updated ==> ORL'.PHP_EOL; fwrite($this->myfile, $txt);
        }
        else
        {
            unset( $this->reg['id'] );
            $this->setId( $this->db->insertArray( $this->table, $this->reg ) );
        }

        $this->loadSpecialFields();

//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return $this->getId();
    }

    /**
     *
     * Delete this record
     *
     */
    public function deleteORL()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==========> ('.$this->table.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->deleteORL( $this->table, $this->user, 'id', $this->getId() );
//$txt = 'reg deleted ==========> '.$this->reg['id'].PHP_EOL; fwrite($this->myfile, $txt);
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return true;
        }
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return false;
    }

    /**
     *
     * Delete this record
     *
     */
    public function delete()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Table ==========> ('.$this->table.')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        if ( $this->reg['id'] != '' && $this->reg['id'] != '0' )
        {
            $this->db->delete( $this->table, 'id', $this->getId() );
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
            return true;
        }
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        return false;
    }

    public function setReg( $reg ) { $this->reg = array_merge( $this->reg, $reg );  }
    public function setId( $id ) { $this->reg['id'] = $id;  }
    public function setAccount( $account ) { $this->reg['account'] = $account;  }
    public function setDate( $date ) { $this->reg['date'] = $date;  }
    public function setQuoteKey( $quotekey ) { $this->reg['quote_key'] = $quotekey;  }
    public function setNet( $net ) { $this->reg['net'] = $net;  }
    public function setVatAmount( $vatamount ) { $this->reg['vat_amount'] = $vatamount;  }
    public function setTotalToPay( $totaltopay ) { $this->reg['total_to_pay'] = $totaltopay;  }
    public function setQuoteType( $quotetype ) { $this->reg['quote_type'] = $quotetype;  }
    public function setInvoice( $invoice ) { $this->reg['invoice'] = $invoice;  }
    public function setPaymentType( $payment_type ) { $this->reg['payment_type'] = $payment_type;  }
    public function setPaymentMethod( $payment_method ) { $this->reg['payment_method'] = $payment_method;  }
    public function setAccountPaymentMethod( $account_payment_method ) { $this->reg['payment_method'] = $account_payment_method;  }
    public function setPaymentOrigin( $payment_origin ) { $this->reg['payment_origin'] = $payment_origin;  }
    public function setPaymentReference( $payment_reference ) { $this->reg['payment_reference'] = $payment_reference;  }

    public function getReg() { return $this->reg; }
    public function getAll( $filter='', $extra='' ) { return $this->db->fetchAll( $this->table, '*', $filter, $extra ); }
    public function getId() { return $this->reg['id']; }
    public function getAccount() { return $this->reg['account']; }
    public function getDate() { return $this->reg['date']; }
    public function getQuoteKey() { return $this->reg['quote_key']; }
    public function getNet() { return $this->reg['net']; }
    public function getVatAmount() { return $this->reg['vat_amount']; }
    public function getTotalToPay() { return $this->reg['total_to_pay']; }
    public function getQuoteType() { return $this->reg['quote_type']; }
    public function getInvoice() { return $this->reg['invoice']; }
    public function getPaymentType() { return $this->reg['payment_type']; }
    public function getPaymentMethod() { return $this->reg['payment_method']; }
    public function getAccountPaymentMethod() { return $this->reg['payment_method']; }
    public function getPaymentOrigin() { return $this->reg['payment_origin']; }
    public function getPaymentReference() { return $this->reg['payment_reference']; }

    /**
     *
     * Change special fields after load from database
     */
    private function loadSpecialFields()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        $this->reg['date'] = ( empty($this->reg['date']) )? NULL : DateTime::createFromFormat('Y-m-d', $this->reg['date'], new DateTimeZone($this->session->config['time_zone']));
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
    }

    /**
     *
     * Change special fields before save to database
     */
    private function setSpecialFields()
    {
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        $this->reg['date'] = ( empty($this->reg['date']) )? NULL : $this->reg['date']->format('Y-m-d');

        $this->reg['account'] = ( empty($this->reg['account']) )? NULL : $this->reg['account'];
        $this->reg['invoice'] = ( empty($this->reg['invoice']) )? NULL : $this->reg['invoice'];
        $this->reg['quote_type'] = ( empty($this->reg['quote_type']) )? NULL : $this->reg['quote_type'];
        $this->reg['payment_type'] = ( empty($this->reg['payment_type']) )? NULL : $this->reg['payment_type'];
        $this->reg['payment_method'] = ( empty($this->reg['payment_method']) )? NULL : $this->reg['payment_method'];

        $this->reg['net'] = ( empty($this->reg['net']) )? '0' : $this->reg['net'];
        $this->reg['vat_amount'] = ( empty($this->reg['vat_amount']) )? '0' : $this->reg['vat_amount'];
        $this->reg['total_to_pay'] = ( empty($this->reg['total_to_pay']) )? '0' : $this->reg['total_to_pay'];
//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
    }
}