...
class credentialDeleteViewController extends baseViewController
{
    ...
    /**
     * @Route("/app/credential/delete/id", name="app_credential_delete")
     *
     * @param POST
     *
    ...
    {
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/credentialDeleteViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $form_action = $this->folder.'/credential/delete';
        ...
        if ( !isset( $vars['id']) || $vars['id'] == '' || $vars['id'] == '0' )
        {
            $_SESSION['alert'] = array(
                'type'          => 'danger',
                'message'       => $this->lang['ERR_CREDENTIAL_NOT_EXISTS'],
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            $filter_select = '';
            $extra_select = '';
            $workflows = $workflow->getAll( $filter_select, $extra_select );
            foreach ( $workflows as $row )
            {
                $workflow->getRegbyId($row['id']);

                if ( sizeof( $workflow->getCredentials() ) )
                {
                    foreach ( $workflow->getCredentials() as $credential_temp )
                    {
                        if ( $credential_temp == $reg->getId() )
                        {
                            $found = true;
                            continue;
                        }
            ...
            if ( $found )
            {
                $error_ajax[] = array (
                                        'dom_object' => [''],
                                        'msg' => $this->lang['ERR_CREDENTIAL_IS_IN_WORKFLOW'],
                );
            }

            if ( !sizeof( $error_ajax ) )
            {
                $this->logger->info('==============='.__METHOD__.' Credential '.$vars['id'].' | User '.$this->user.' ===================================================');
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['CREDENTIAL_DELETED'];
