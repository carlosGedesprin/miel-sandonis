$(document).ready(function() {
    ...
    $('#item-form :input:enabled:visible:first').focus();
...
$(function() {
    ...
    $(".dropBox")
        ...
        .on("drop", function(e) {
            ...
            var max_size_mb = file_max_size / 1024  / 1024 ;

            if ( checkFileSize( file, max_size_mb ) )
            {
                fd.append('file_input_'+item, file);
                showPreview( file, item );
                $('#file-delete-' + item).show();
            }
            else
            {
                e.preventDefault();
                const errorMessage = l_file_max_size.replace('%1$s', max_size_mb.toLocaleString());
                alert(errorMessage);
            }
        })
    ;
    ...
    $("#btn_cancel").click(function(){
        //console.log('Cancel route', cancel);
    ...
    $("#btn_submit").click(function(){
        ...
        $(document.body).prepend("<div id='loading'><img src='/assets/images/app/default/loading_icon.webp'></div>");

        //$("#loading").css("height", $('body').outerHeight(true));
        $("#loading").css("height", '200vh');
        ...
        $( "form :input" ).each(function()
        {
            ...
            if ( this.type === 'checkbox' ) {
                fd.append(name, this.checked ? value : '0');
                return;
            }
        ...
        else
        {
            ...
            url_form = '/' + folder + '/' + entity + '/' + action + '/' + reg_id;
        }
        ...
        $.ajax({
            ...
            */
            ...
            .done(function(response){
                ...
                //console.log('JSON parsed', json_resp);
                ...
                if( json_resp.status == 'OK' )
                {
                    ...
                    $('#box-result').find( 'div' ).addClass("alert-success").append($("<p id='box_result_msg'>").addClass("text-center") );
                    $('#box_result_msg').html( json_resp.msg );

                    showResult();

                    let message_time_out;
                    ( !("message_time_out" in json_resp) )? message_time_out = 2500 : message_time_out = json_resp.message_time_out;
                    setTimeout(function(){
                        $('#item-form').attr('action', json_resp.action);
                        // console.log($('#item-form').attr('action'));
                        $('#item-form').submit();
                    }, message_time_out);
                }
                else if( json_resp.status == 'KO' )
                {
                    ...
                    $.each(json_resp.errors, function(index, error)
                    {
                        ...
                        $('#box-result').find('ul').append($("<li>").html(error.msg));
                ...
                else if( json_resp.status == 'CONTINUE' )
                {
                    ...
                    let url_to_continue = json_resp.url;
    ...
    /**
     * Verifies if file has max file size allowed
     * @param {File} file - File to validate.
     * @param {number} maxSizeMB - Max amount in Mb allowed.
     * @returns {boolean} - true if it is a valid file false if not.
     */
    function checkFileSize(file, maxSizeMB) {
        ...
        // console.log("Max file size allowed (MB):", maxSizeMB);
    ...
    function showResult()
    {
        ...
        if ( $(".card").length )
        {
            ...
            $('.is_wrong').each( function( index, node )
            {
                ...
                // console.log( $( this ) );
                var $closest_pane = $(this).closest('.tab-pane');
                var $closest_pane_id = $closest_pane.attr('id');
                // console.log( 'Wrong is in pane with id ('+$closest_pane_id+')' );
                var $tab_needle = $closest_pane_id.replace('tab-','');
                // console.log( 'Needle of tab ('+$tab_needle+')' );

                // Find the pane tag (li link) that corresponds to the pane and mark it with errors
                var $tab_li = $( '#'+$tab_needle+'-tab' );
                // console.log( 'Tab li a ('+$tab_li.attr('id')+')' );
                $( $tab_li ).css('background-color', 'rgba(255,0,0,0.60)');
                $( $tab_li ).css('color', 'rgba(255,255,255,1)');
            ...
            if ( $tab_to_show != '' )
            {
                ...
                // console.log('tab to show', $( '#'+$tab_to_show+'-tab' ).id);
            }
        }
        else
        {
