...
/**
 * Trait automation verifications
 * @package entity
 */
trait automationVerificationController
{

    /*
     *
     * Check if another automation has the same name
     *
     * $name : name to search
     * $type_key : '', 'id' or 'key' key to avoid the self record
     * $automation_id : id or automation_key referred on the $type_key
     * $account: Name not duplicated per account, expected account id
     */
    public function automationsWithSameName( $name, $type_key, $key, $account )
    {
...
//$txt = 'Filters ==> '.print_r($filters, true).PHP_EOL . PHP_EOL; fwrite($this->myfile, $txt);
        $extra = ( $key == 0 )? '' : ' AND '.( ( $type_key == 'id' )? 'id' : 'automation_key').' <> \''.$key.'\'';
