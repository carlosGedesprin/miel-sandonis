...
class N8NWarmIPAccountEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/n8n_warm_ip_account/edit/id', name='app_n8n_warm_ip_account_edit')
     *
     * @param POST
     *
    ...
    {
        $this->logger->info('==============='.__METHOD__.' N8N Warm IP Account process '.$vars['id'].' | User '.$this->user.' ===================================================');
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/N8NWarmIPAccountEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $now = new DateTime('now', new DateTimeZone($this->session->config['time_zone']) );

        $form_action = $this->folder.'/n8n_warm_ip_account/editor';

        $this->pagination = $this->utils->request_pagination( $this->pagination );
        list( $this->list_filters, $this->pagination['num_page'] )  = $this->utils->request_filters( $this->list_filters, $this->pagination['num_page'] );

        $reg = new N8NWarmIPAccount( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );
        ...
        if ( $data['submit'] )
        {
            ...
            if (!$valid) {
                return $this->twig->render('app/'.$this->session->config['app_skin'].'/common/show_message.html.twig', array(
                    'section' => $this->lang['N8N_WARM_IP_ACCOUNT_EDIT'],
                    'alert_type' => 'danger',
            ...
            */
            ...
            else
            {
                ...
                else if ( $reg->accountsWithSameEmail( $reg->getEmail(), 'id', $reg->getId() ) )
                {
                    $error_ajax[] = array (
                        'dom_object' => ['email'],
            ...
            if ( !sizeof( $error_ajax ) )
            {
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['N8N_WARM_IP_ACCOUNT_SAVED'];
                $response['action'] = '/'.$this->folder.'/'.$this->lang['N8N_WARM_IP_ACCOUNTS_LINK'];
...
//fwrite($this->myfile, print_r($response['errors'], TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $_SESSION['alert'] = array('type'=>'danger', 'message'=>$this->lang['ERR_N8N_WARM_IP_ACCOUNT_NOT_EXISTS']);
                    header('Location: /'.$this->folder.'/'.$this->lang['N8N_WARM_IP_ACCOUNTS_LINK']);
                    exit;
                }
            }
        }
...
//fclose($this->myfile);
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/N8N_warm_ip_accountForm.html.twig', array(
            'reg' => $reg->getReg(),
            'filters' => $this->list_filters,
            'pagination' => $this->pagination,
            'data' => $data,
            'cancel' => '/'.$this->folder.'/'.$this->lang['N8N_WARM_IP_ACCOUNTS_LINK'],
        ));
    }
}