...
class leadOriginEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/lead_origin/edit/id', name='app_lead_origin_edit')
     *
     * @param POST
     *
    ...
    {
        $this->logger->info('==============='.__METHOD__.' Lead origin process '.$vars['id'].' | User '.$this->user.' ===================================================');
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/leadOriginEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $now = new DateTime('now', new DateTimeZone($this->session->config['time_zone']) );

        $form_action = $this->folder.'/lead_origin/editor';

        $this->pagination = $this->utils->request_pagination( $this->pagination );
        list( $this->list_filters, $this->pagination['num_page'] )  = $this->utils->request_filters( $this->list_filters, $this->pagination['num_page'] );

        $reg = new leadOriginController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            if ( empty( $reg->getLangKey() ) )
            {
                $error_ajax[] = array (
                    'dom_object' => ['lang_key'],
                    'msg' => $this->lang['ERR_LEAD_ORIGIN_LANG_KEY_NEEDED'],
                );
            }
            else
            {
                if ( strlen( $reg->getLangKey() ) < 2 )
                {
                    $error_ajax[] = array (
                        'dom_object' => ['lang_key'],
                        'msg' => sprintf( $this->lang['ERR_LEAD_ORIGIN_LANG_KEY_SHORT'], '2' ),
                    );
                }
                else if ( strlen( $reg->getLangKey() ) > 50 )
                {
                    $error_ajax[] = array (
                        'dom_object' => ['lang_key'],
                        'msg' => sprintf( $this->lang['ERR_LEAD_ORIGIN_LANG_KEY_LONG'], '50' ),
                    );
                }
                else
                {
                    ...
                    if ( $this->db->fetchOne( $reg->getTableName(), 'id', ['lang_key' => $reg->getLangKey()], ' AND id <> '.$reg->getId()) )
                    {
                        $error_ajax[] = array (
                            'dom_object' => ['lang_key'],
                            'msg' => $this->lang['ERR_LEAD_ORIGIN_LANG_KEY_EXISTS'],
                        );
                    }
                }
            }
...
//$txt = '========== EDIT =========='.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['LEAD_ORIGIN_SAVED'];
                $response['action'] = '/'.$this->folder.'/lead_origins';
...
//$txt = '========== EDIT =========='.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                {
                    $_SESSION['alert'] = array('type'=>'danger', 'message'=>$this->lang['ERR_LEAD_ORIGIN_NOT_EXISTS']);
                    header('Location: /'.$this->folder.'/'.$this->lang['LEAD_ORIGINS_LINK']);
                    exit;
                }
            }
        }
...
//fclose($this->myfile);
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/leadOriginForm.html.twig', array(
            'reg' => $reg->getReg(),
            'filters' => $this->list_filters,
            'pagination' => $this->pagination,
            'data' => $data,
            'cancel' => '/'.$this->folder.'/lead_origins',
        ));
    }
}