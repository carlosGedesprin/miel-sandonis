...
class categoryEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/category/edit/id', name='app_category_edit')
     *
     * @param POST
     *
    ...
    {
        $this->logger->info('==============='.__METHOD__.' Category '.$vars['id'].' | User '.$this->user.' ===================================================');
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/categoryEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $form_action = $this->folder.'/category/editor';

        $this->pagination = $this->utils->request_pagination( $this->pagination );
        list( $this->list_filters, $this->pagination['num_page'] )  = $this->utils->request_filters( $this->list_filters, $this->pagination['num_page'] );

        $reg = new categoryController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );
        $reg_original = new categoryController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );

        $reg->setId( ( isset( $vars['id'] ) )? $vars['id'] : '0' );
        //$reg->setCategoryKey( $this->utils->request_var( 'key', '', 'ALL', true) );
...
//$txt = 'On start function end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        ...
        //c:\xampp\htdocs\proaccesible\web\bundles\framework\documents\categories\
        $files_folder = DOCUMENT_APP_ROOT_PATH.'/categories/';
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            else
            {
                ...
                {
                    if ( $reg->categoriesWithSameName( $reg->getName(), $reg->getId() ) )
                    {
                        $error_ajax[] = array (
                            'dom_object' => ['name'],
            ...
            if ( empty( $reg->getSlug() ) )
            {
                $error_ajax[] = array (
                    'dom_object' => ['slug'],
                    'msg' => $this->lang['ERR_CATEGORY_SLUG_NEEDED'],
                );
            }
            else
            {
                if ( strlen( $reg->getSlug() ) < 2 )
                {
                    $error_ajax[] = array (
                        'dom_object' => ['slug'],
                        'msg' => sprintf( $this->lang['ERR_CATEGORY_SLUG_SHORT'], '2' ),
                    );
                }
                else if ( strlen( $reg->getSlug() ) > 20 )
                {
                    $error_ajax[] = array (
                        'dom_object' => ['slug'],
                        'msg' => sprintf( $this->lang['ERR_CATEGORY_SLUG_LONG'], '20' ),
                    );
                }
                elseif ( $reg->categoriesWithSameSlug( $reg->getSlug(), $reg->getId() ) )
                {
                    $error_ajax[] = array (
                        'dom_object' => ['slug'],
                        'msg' => $this->lang['ERR_CATEGORY_SLUG_EXISTS'],
                    );
                }
            }
...
//$txt = '================= Moving images from temp to files destiny folder end =============================================='.PHP_EOL.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                if ( $data['action'] == 'add' )
                {
                    ...
                    $reg->setCategoryKey( md5( $reg->getId() ) );
                    $reg->persistORL();
                }
                else
                {
                    ...
                    $reg->setCategoryKey( $reg_original->getCategoryKey() );
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['CATEGORY_SAVED'];
                $response['action'] = '/'.$this->folder.'/'.$this->lang['CATEGORIES_LINK'];
...
//$txt = '================= Move from user folder to temp folder end =============================================='.PHP_EOL.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                { 
                    $_SESSION['alert'] = array(
                                               'type'=>'danger',
                                               'message'=>$this->lang['ERR_CATEGORY_NOT_EXISTS']
                    );
                    header('Location: /'.$this->folder.'/'.$this->lang['CATEGORIES_LINK']);
                    exit();
                }
            }
        }
...
//fclose( $this->myfile );
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/categoryForm.html.twig', array(
            'reg' => $reg->getReg(),
            ...
            'files' => $files,
            //********* File treatment end ******************
            'cancel' => '/'.$this->folder.'/'.$this->lang['CATEGORIES_LINK'],
        ));
    }
}
