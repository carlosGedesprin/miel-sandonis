...
use \src\controller\entity\subSectorController;
use \src\controller\entity\sectorController;

use DateTime;
use DateTimeZone;

class subSectorEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/sub-sector/edit/id', name='app_sub_sector_edit')
     *
     * @param POST
     *
    ...
    {
        $this->logger->info('==============='.__METHOD__.' Sub sector '.$vars['id'].' | User '.$this->user.' ===================================================');
$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/subSectorEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $form_action = $this->folder.'/sub_sector/editor';

        $this->pagination = $this->utils->request_pagination( $this->pagination );
        list( $this->list_filters, $this->pagination['num_page'] )  = $this->utils->request_filters( $this->list_filters, $this->pagination['num_page'] );

        $reg = new subSectorController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );
        $reg_original = new subSectorController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );

        $reg->setId( ( isset( $vars['id'] ) )? $vars['id'] : '0' );
        $reg->setSector( $this->utils->request_var( 'sector', '', 'ALL') );
...
//$txt = 'On start function end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        ...
        //c:\xampp\htdocs\proaccesible\web\bundles\framework\documents\subsectors\
        $files_folder = DOCUMENT_APP_ROOT_PATH.'/subsectors/';
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            else
            {
                ...
                {
                    if ( $reg->subSectorsWithSameName( $reg->getName(), $reg->getId() ) )
                    {
                        $error_ajax[] = array (
                            'dom_object' => ['name'],
            ...
            else
            {
                ...
                elseif ( $reg->subSectorsWithSameSlug( $reg->getSlug(), $reg->getId() ) )
                {
                    $error_ajax[] = array (
                        'dom_object' => ['slug'],
...
//$txt = '================= Moving images from temp to files destiny folder end =============================================='.PHP_EOL.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['SUB_SECTOR_SAVED'];
                $response['action'] = '/'.$this->folder.'/'.$this->lang['SUB_SECTORS_LINK'];
...
//$txt = '================= Move from user folder to temp folder end =============================================='.PHP_EOL.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                { 
                    $_SESSION['alert'] = array(
                                               'type'=>'danger',
                                               'message'=>$this->lang['ERR_SUB_SECTOR_NOT_EXISTS']
                    );
                    header('Location: /'.$this->folder.'/'.$this->lang['SUB_SECTORS_LINK']);
                    exit();
                }
            }
        }
        ...
        // Sector options
        if ( empty( $reg->getSector() ) )
        {
            $data['sector_options'] .= '<option value="" selected="selected">'.$this->lang['CATEGORY_SELECT'].'</option>';
            $data['sector_options'] .= '<option disabled>'.str_repeat('&#x2500', 16).'</option>';
        }
        if ( $data['action'] == 'add' )
        {
            $data['sector_options'] .= '<option value="-"'.(($reg->getCategory() === '-')? ' selected="selected" ' : '').'>'.$this->lang['CATEGORY_ALL'].'</option>';
            $data['sector_options'] .= '<option disabled>'.str_repeat('&#x2500', 16).'</option>';
        }
        $filter_select = array(
            'active' => '1'
        ...
        foreach ( $rows as $row)
        {
            $data['sector_options'] .= '<option value="'.$row['id'].'"'.(($reg->getSector() == $row['id'])? ' selected="selected" ' : '').'>'.$row['name'].'</option>';
        }

//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//fclose( $this->myfile );
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/subSectorForm.html.twig', array(
            'reg' => $reg->getReg(),
            ...
            'files' => $files,
            //********* File treatment end ******************
            'cancel' => '/'.$this->folder.'/'.$this->lang['SUB_SECTORS_LINK'],
        ));
    }
}
