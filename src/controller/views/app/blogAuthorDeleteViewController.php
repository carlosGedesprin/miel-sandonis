...
class blogAuthorDeleteViewController extends baseViewController
{
    ...
    /**
     * @Route("/app/blog_author/delete/id", name="app_blog_author_delete")
     *
    ...
    {
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/blogAuthorDeleteViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $form_action = $this->folder.'/blog_author/delete';
        ...
        if ( !isset( $vars['id']) || $vars['id'] == '' || $vars['id'] == '0' )
        {
            $_SESSION['alert'] = array(
                'type'          => 'danger',
                'message'       => $this->lang['ERR_BLOG_AUTHOR_NOT_EXISTS'],
                'filters'       => $this->list_filters,
                'pagination'    => $this->pagination,
            );
            header('Location: /'.$this->folder.'/'.$this->lang['BLOG_AUTHORS_LINK']);
            exit;
        }

        $reg = new blogAuthorController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            if ( sizeof( $blog_articles ) )
            {
                $error_ajax[] = array (
                    'dom_object' => [],
                    'msg' => $this->lang['ERR_BLOG_AUTHOR_HAS_ARTICLES'],
                );
            }

            if ( !sizeof( $error_ajax ) )
            {
                $this->logger->info('==============='.__METHOD__.' Blog author '.$vars['id'].' | User '.$this->user.' ===================================================');
...
//$txt = '================= Delete from destiny folder end =============================================='.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['BLOG_AUTHOR_DELETED'];
                $response['action'] = '/'.$this->folder.'/'.$this->lang['BLOG_AUTHORS_LINK'];
...
//fwrite($this->myfile, print_r($reg->getReg(), TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
            ...
            {
                $_SESSION['alert'] = array('type'=>'danger', 'message'=>$this->lang['ERR_BLOG_AUTHOR_NOT_EXISTS']);
                header('Location: /'.$this->folder.'/'.$this->lang['BLOG_AUTHORS_LINK']);
                exit();
            }
        }
...
//fclose($this->myfile);
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/blog_authorForm.html.twig', array(
            'reg' => $reg->getReg(),
            'filters' => $this->list_filters,
            'pagination' => $this->pagination,
            'data' => $data,
            ...
            'files' => $files,
            //********* File treatment end ******************
            'cancel' => '/'.$this->folder.'/'.$this->lang['BLOG_AUTHORS_LINK'],
        ));
    }
}
