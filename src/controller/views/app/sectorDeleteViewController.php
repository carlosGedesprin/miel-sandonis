...
class sectorDeleteViewController extends baseViewController
{
    ...
    /**
     * @Route("/app/sector/delete/id", name="app_sector_delete")
     *
     * @param POST
     *
    ...
    {
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/sectorDeleteViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $form_action = $this->folder.'/sector/delete';
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            if ( !sizeof( $error_ajax ) )
            {
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['SECTOR_DELETED'];
...
//fwrite($this->myfile, print_r($reg->getReg(), TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
            ...
            {
                $_SESSION['alert'] = array('type'=>'danger', 'message'=>$this->lang['ERR_SECTOR_NOT_EXISTS']);
                header('Location: /'.$this->folder.'/'.$this->lang['SECTORS_LINK']);
                exit();
            }
        }
