...
use \src\controller\entity\solutionController;
use \src\controller\entity\workflowController;

use DateTime;
use DateTimeZone;

class solutionEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/solution/edit/id', name='app_solution_edit')
     *
     * @param POST
     *
    ...
    {
        $this->logger->info('==============='.__METHOD__.' Solution process '.$vars['id'].' | User '.$this->user.' ===================================================');
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/solutionEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $now = new DateTime('now', new DateTimeZone($this->session->config['time_zone']) );

        $form_action = $this->folder.'/solution/editor';

        $this->pagination = $this->utils->request_pagination( $this->pagination );
        list( $this->list_filters, $this->pagination['num_page'] )  = $this->utils->request_filters( $this->list_filters, $this->pagination['num_page'] );

        $reg = new solutionController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );
        $reg_original = new solutionController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );

        $workflow = new workflowController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );

        $reg->setId(( isset( $vars['id'] ) )? $vars['id'] : '0');
        $reg->setName( $this->utils->request_var( 'name', '', 'ALL') );
        $reg->setSlug( $this->utils->request_var( 'slug', '', 'ALL') );
        $reg->setTitle( $this->utils->request_var( 'title', '', 'ALL') );
        $reg->setSubTitle( $this->utils->request_var( 'subtitle', '', 'ALL') );
        $reg->setProblem( $this->utils->request_var( 'problem', '', 'ALL') );
        $reg->setSolution( $this->utils->request_var( 'solution', '', 'ALL') );
        //$reg->setImage( $this->utils->request_var( 'image', '', 'ALL') );
        ...
        $data = array(
            ...
            'workflows'     => $this->utils->request_var( 'workflows', '', 'ALL'),
            'workflows_options' => '',
        );
...
//$txt = 'On start function end ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
        ...
        $files_folder = DOCUMENT_ROOT_PATH.'/solutions/';
        ...
        $files = array(
            '1' => array (
                ...
                'image_error_text' => $this->lang['ERR_SOLUTION_IMAGE_NEEDED'],
                'file_entity' => 'reg',
                'file_entity_method' => 'Image',
            ),
        );
        ...
        if ( $data['submit'] )
        {
            ...
            if (!$valid) {
                return $this->twig->render('app/'.$this->session->config['app_skin'].'/common/show_message.html.twig', array(
                    'section' => $this->lang['SOLUTION_EDIT'],
                    'alert_type' => 'danger',
            ...
            */
            ...
            if ( empty( $reg->getProblem() ) )
            {
                $error_ajax[] = array (
                    'dom_object' => ['problem'],
                    'msg' => $this->lang['ERR_SOLUTION_PROBLEM_NEEDED'],
                );
            }

            if ( empty( $reg->getSolution() ) )
            {
                $error_ajax[] = array (
                    'dom_object' => ['solution'],
                    'msg' => $this->lang['ERR_SOLUTION_SOLUTION_NEEDED'],
                );
            }
...
//$txt = '========== NEW =========='.PHP_EOL; fwrite($this->myfile, $txt);
                    ...
                    $reg->setKey( md5( $reg->getId().$reg->getName() ) );
                    $reg->persist();
                }
                else
                {
...
//$txt = 'Workflows ======= '.$data['workflows'].PHP_EOL; fwrite($this->myfile, $txt);
                $data['workflows'] = explode(",", $data['workflows']);
//$txt = 'Workflows array ======= '.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($data['workflows'], TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
                $reg->cleanWorkflows();
                foreach ( $data['workflows'] as $workflow_value )
                {
//$txt = 'Workflow to treat ======= '.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($workflow_value, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
                    $workflow->getRegbyId( $workflow_value );

                    $reg->setWorkflow( $workflow->getId() );
                }
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['SOLUTION_SAVED'];
                $response['action'] = '/'.$this->folder.'/'.$this->lang['SOLUTIONS_LINK'];
...
//fwrite($this->myfile, print_r($files, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
                ...
                {
                    $_SESSION['alert'] = array('type'=>'danger', 'message'=>$this->lang['ERR_SOLUTION_NOT_EXISTS']);
                    header('Location: /'.$this->folder.'/'.$this->lang['SOLUTIONS_LINK']);
                    exit;
                }
            }
        }

        // Workflows select options list
        $filter_select = [
                            'active' => '1'
                        ];
        $extra_select = 'ORDER BY `name`';
        $data_options_field = 'workflows_options';
        $workflow = new workflowController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );
        $rows = $workflow->getAll( $filter_select, $extra_select );
        if ( empty( $reg->getWorkflows() ) )
        {
        ...
        foreach ( $rows as $row )
        {
            $workflow->getRegbyId( $row['id'] );

            $data['workflows_options'] .= '<option value="' . $workflow->getId() . '"';

            if ( in_array( $workflow->getId(), $reg->getWorkflows()) )
            {
                $data['workflows_options'] .= ' selected="selected" ';
            }

            $data['workflows_options'] .= '>' . $workflow->getName() . '</option>';
        }

//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL;fwrite($this->myfile, $txt);
//fclose($this->myfile);
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/solutionForm.html.twig', array(
            'reg' => $reg->getReg(),
            ...
            'files' => $files,
            //********* File treatment end ******************
            'cancel' => '/'.$this->folder.'/'.$this->lang['SOLUTIONS_LINK'],
        ));
    }
}