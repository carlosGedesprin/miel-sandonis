...
use \src\controller\entity\botController;

use DateTime;
use DateTimeZone;

class botEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/bot/edit/id', name='app_bot_edit')
     *
     * @param POST
     *
    ...
    {
        $this->logger->info('==============='.__METHOD__.' Bot process '.$vars['id'].' | User '.$this->user.' ===================================================');
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/botEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $now = new DateTime('now', new DateTimeZone($this->session->config['time_zone']) );

        $form_action = $this->folder.'/bot/editor';

        $this->pagination = $this->utils->request_pagination( $this->pagination );
        list( $this->list_filters, $this->pagination['num_page'] )  = $this->utils->request_filters( $this->list_filters, $this->pagination['num_page'] );

        $reg = new botController( array( 'env' => $this->env, 'logger' => $this->logger, 'logger_err' => $this->logger_err, 'startup' => $this->startup, 'db' => $this->db, 'utils' => $this->utils, 'session' => $this->session, 'lang' => $this->lang ) );

        $reg->setId(( isset( $vars['id'] ) )? $vars['id'] : '0');
        $reg->setName( $this->utils->request_var( 'name', '', 'ALL') );
        $reg->setUserAgent( $this->utils->request_var( 'user_agent', '', 'ALL') );
        $reg->setIp( $this->utils->request_var( 'ip', '', 'ALL') );
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            if ( !sizeof( $error_ajax ) )
            {
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['BOT_SAVED'];
                $response['action'] = '/'.$this->folder.'/'.$this->lang['BOTS_LINK'];
...
//fwrite($this->myfile, print_r($response['errors'], TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    $_SESSION['alert'] = array('type'=>'danger', 'message'=>$this->lang['ERR_BOT_NOT_EXISTS']);
                    header('Location: /'.$this->folder.'/'.$this->lang['BOTS_LINK']);
                    exit;
                }
            }
        }
...
//fclose($this->myfile);
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/botForm.html.twig', array(
            'reg' => $reg->getReg(),
            'filters' => $this->list_filters,
            'pagination' => $this->pagination,
            'data' => $data,
            'cancel' => '/'.$this->folder.'/'.$this->lang['BOTS_LINK'],
        ));
    }
}