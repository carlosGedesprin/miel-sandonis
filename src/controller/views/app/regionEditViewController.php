...
class regionEditViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/region/edit/id', name='app_region_edit')
     *
     * @param POST
     *
...
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/regionEditViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
...
//fwrite($this->myfile, print_r($vars, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        ...
        $form_action = $this->folder.'/region/editor';
        ...
        $reg->setRegionCode( $this->utils->request_var( 'region_code', ($vars['region_code'] == '0')? '' : $vars['region_code'], 'ALL') );
        $reg->setCountryCode2a( $this->utils->request_var( 'country_code_2a', '', 'ALL') );
        $reg->setIsoCode( $this->utils->request_var( 'iso_code', '', 'ALL', true) );
        ...
        //$reg_name->setRegionCode( $reg->getRegionCode() );
        ...
        $data = array(
                        ...
                        'action' => ( $reg->getRegionCode() == '' )? 'add' : 'edit',
...
//fwrite($this->myfile, print_r($data['names'], TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
        ...
        $files_folder = DOCUMENT_ROOT_PATH.'/regions/';
        ...
        $files = array(
            '1' => array (
                ...
                'image_error_text' => $this->lang['ERR_REGION_FLAG_NEEDED'],
                'file_entity' => 'reg',
...
//fwrite($this->myfile, print_r($data['names'], TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
            ...
            {
//$txt = 'Name to check ====== lang_code_2a ('.$value['lang_code_2a'].') name ('.$value['region_name'].')'.PHP_EOL; fwrite($this->myfile, $txt);
                $reg_name->setRegionCode( $reg->getRegionCode() );
                $reg_name->setLang2a( $value['lang_code_2a'] );
                $reg_name->setName( $value['region_name'] );
...
//$txt = 'Deleting names in table =========='.PHP_EOL; fwrite($this->myfile, $txt);
                //$reg_name->deleteByRegionCode( $reg->getRegionCode() );
...
//$txt = 'Lang to treat ====> ('.$row['code_2a'].')'.PHP_EOL; fwrite($this->myfile, $txt);
                    ...
                    $reg_name->setName( $data['names'][$row['code_2a']]['region_name'] );
...
//$txt = '================= Moving images from temp to files destiny folder end =============================================='.PHP_EOL.PHP_EOL; fwrite($this->myfile, $txt);
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['REGION_SAVED'];
...
//$txt = 'Region can not be saved reg -> Status '.$check_reg['status'].PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($check_reg, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Region can not be saved reg_name -> '.$check_reg_name['status'].PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($check_reg_name, TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
...
//$txt = 'Lang to treat ('.$lang_value['code_2a'].')'.PHP_EOL; fwrite($this->myfile, $txt);
                    $data['names'][$lang_value['code_2a']]['lang_code_2a'] = $lang_value['code_2a'];
                    $data['names'][$lang_value['code_2a']]['region_name'] = '';
                }
                //********* Names treatment openges end ******************
            }
            else
            {
//$txt = '========== EDIT =========='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'Reg to edit =========> '.$reg->getRegionCode().PHP_EOL; fwrite($this->myfile, $txt);

                if ( $reg->getRegbyRegionCode( $reg->getRegionCode() ) )
                {
...
//$txt = 'Region '.$reg->getRegionCode().' name in ('.$lang_value['code_2a'].') is ('.$reg_name->getName().')'.PHP_EOL; fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($reg_name->getReg(), TRUE)); $txt = PHP_EOL; fwrite($this->myfile, $txt);
