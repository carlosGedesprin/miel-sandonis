...
class product_typeDeleteViewController extends baseViewController
{
    ...
    /**
     * @Route("/app/product_type/delete/id", name="app_product_type_delete")
     *
     * @param POST
     *
    ...
    {
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/product_typeControler_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__FUNCTION__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);

        $form_action = $this->folder.'/product_type/delete';
        ...
        if ( !isset( $vars['id']) || $vars['id'] == '' || $vars['id'] == '0' )
        {
            $_SESSION['alert'] = array(
                'type'          => 'danger',
                'message'       => $this->lang['ERR_PRODUCT_TYPE_NOT_EXISTS'],
        ...
        if ( $data['submit'] )
        {
            ...
            */
            ...
            if ( sizeof( $product->getAll( ['product_type' => $reg->getId()] ) ) )
            {
                $error_ajax[] = array (
                    'dom_object' => [],
                    'msg' => $this->lang['ERR_PRODUCT_TYPE_HAS_PRODUCTS'],
                );
            }
            ...
            if ( !sizeof( $error_ajax ) )
            {
                ...
                $response['status'] = 'OK';
                $response['msg'] = $this->lang['PRODUCT_TYPE_DELETED'];
