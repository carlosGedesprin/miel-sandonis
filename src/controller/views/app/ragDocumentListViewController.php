...
class ragDocumentListViewController extends baseViewController
{
    ...
    /**
     * @Route('/app/rag-document', name='app_rag_document')
     */
    public function itemslistAction()
    {
//$this->myfile = fopen(APP_ROOT_PATH.'/var/logs/ragDocumentListViewController_'.__FUNCTION__.'.txt', 'w') or die('Unable to open file!');
//$txt = '====================== '.__METHOD__.' start ==============================================================='.PHP_EOL; fwrite($this->myfile, $txt);
//$txt = 'this->pagination ========>'.PHP_EOL;fwrite($this->myfile, $txt);
//fwrite($this->myfile, print_r($this->pagination, TRUE));
        $this->list_filters['name']['caption'] = $this->lang['RAG_DOCUMENT_NAME'];
        $this->list_filters['name']['placeholder'] = $this->lang['RAG_DOCUMENT_NAME'];
        $this->list_filters['rag']['caption'] = $this->lang['RAG_NAME'];
        $this->list_filters['rag']['placeholder'] = $this->lang['RAG_NAME'];
        ...
        // Filter rag select options list
        $filter_options = '';
        $filter_options .= '<option value=""'.(($this->list_filters['rag']['value'] == '')? ' selected="selected" ' : '').'>'.(($this->list_filters['rag']['value'] == '0')? $this->lang['RAG_SELECT'] : $this->lang['RAG_ALL']).'</option>';
        ...
        foreach ( $rows as $row)
        {
            $filter_options .= '<option value="' . $row['id'] . '"' . (($this->list_filters['rag']['value'] == $row['id']) ? ' selected="selected" ' : '') . '>'.$row['name'].' - '.$row['id'].'</option>';
        }
        $this->list_filters['rag']['options'] = $filter_options;
        unset($filter_options);

//$txt = '====================== '.__METHOD__.' end ==============================================================='.PHP_EOL;fwrite($this->myfile, $txt);
//fclose($this->myfile);
        return $this->twig->render('app/'.$this->session->config['app_skin'].'/'.$this->folder.'/ragDocuments.html.twig', array(
